...
#define DHT11_PIN 7         // define anlog  port 1
...
byte read_dht11_dat() {
  ...
  for (i = 0; i < 8; i++) {
    ...
    {};  // wait  forever until anlog input port 0 is '1'   (NOTICE: PIND reads all the analog input ports
    ...
    if (PIND & _BV(DHT11_PIN)) //if analog input port 0 is still '1' after 30 us
...
void readDHT() {
  ...
  PORTD &= ~_BV(DHT11_PIN);    // 1. pull-down i/o pin for 18ms
  delay(18);
  PORTD |= _BV(DHT11_PIN);     // 2. pull-up i/o pin for 40us
  delayMicroseconds(1);
  DDRD &= ~_BV(DHT11_PIN);     //let analog port 0 be input port
  delayMicroseconds(40);

  dht11_in = PIND & _BV(DHT11_PIN);  // read only the input port 0
  if (dht11_in) {
  ...
  DDRD |= _BV(DHT11_PIN);      //let analog port 0 be output port after all the data have been received
  PORTD |= _BV(DHT11_PIN);     //let the  value of this port be '1' after all the data have been received
...
void setup() {
  ...
  DDRD |= _BV(DHT11_PIN);   //let analog port 0 be output port
  PORTD |= _BV(DHT11_PIN);  //let the initial value of this port be '1'
  Serial.begin(9600);
